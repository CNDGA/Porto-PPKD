...
if(!isset($_SESSION['email'])){
  ...
  //tampilkan database table blog dan catergoriess hanya nama_kategories dmana table blog menampungkan table categoriees ON(siapa yang mao di selasikan) table blog id_kategori = table categories id
$query = mysqli_query($a, "SELECT blog.*, categories.nama_kategori FROM blog LEFT JOIN categories ON blog.id_kategori = categories.id");
$rows = mysqli_fetch_all($query, MYSQLI_ASSOC);
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];

    $queryBlog  = mysqli_query($a, "SELECT * FROM blog WHERE id = $id");
    $rowBlog = mysqli_fetch_assoc($queryBlog);
    unlink('../assets/uploads/' . $rowBlog['foto']);

    $delete = mysqli_query($a, "DELETE FROM blog WHERE id = $id");
    if ($delete) {
        header("Location: blog.php?delete=berhasil");
    }
}
