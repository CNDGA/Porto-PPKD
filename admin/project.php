...
$query = mysqli_query($a, "SELECT * FROM project");
$rows = mysqli_fetch_all($query, MYSQLI_ASSOC);
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];

    $queryProject  = mysqli_query($a, "SELECT * FROM project WHERE id = $id");
    $rowProject = mysqli_fetch_assoc($queryProject);
    unlink('../assets/uploads/' . $rowProject['foto']);

    $delete = mysqli_query($a, "DELETE FROM project WHERE id = $id");
    if ($delete) {
